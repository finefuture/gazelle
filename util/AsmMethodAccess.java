...
/**
 * 本类来源于开源rpc框架 jupiter
 * 地址：{link https://github.com/fengjiachun/Jupiter}
 *
 * jupiter
 ...
 * @author jiachun.fjc
 */
public abstract class AsmMethodAccess {

    private static final ConcurrentMap<Class<?>, AsmMethodAccess> fastAccessorCache = new ConcurrentHashMap<>();

    private String[] methodNames;
    private Class<?>[][] parameterTypes_s;

    /**
     * 用ASM生成的类继承 {@link AsmMethodAccess} 并实现这个抽象方法
     *
    ...
    public static AsmMethodAccess get(Class<?> type) {
        AsmMethodAccess accessor = fastAccessorCache.get(type);
        if (accessor == null) {
    ...
    private static AsmMethodAccess create(Class<?> type) {
        ...
        String accessorClassName = className + "_AsmMethodAccess";
        String accessorClassNameInternal = accessorClassName.replace('.', '/');
        String classNameInternal = className.replace('.', '/');
        String superClassNameInternal = AsmMethodAccess.class.getName().replace('.', '/');
        ...
        try {
            AsmMethodAccess accessor = (AsmMethodAccess) accessorClass.newInstance();
